...
export interface IAuthRepository {
  ...
  getRefreshToken(token: string): Promise<{ token: string; userId: number; expiresAt: Date } | null>;
  ...
  findEmailVerification(token: string): Promise<{ id: number; userId: number; expiresAt: Date } | null>;
