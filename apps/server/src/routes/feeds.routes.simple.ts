...
router.post('/:id/refresh', async (req: any, res: any) => {
  try {
    ...
    // TODO: Add to BullMQ feed queue for immediate processing
    // For now, just update the feed status to indicate refresh requested
    ...
    try {
      await redisClient.publish('admin:feed:refresh', JSON.stringify({
        feedId,
        feedName: feed.name,
        timestamp: new Date().toISOString(),
