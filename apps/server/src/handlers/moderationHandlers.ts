...
export function registerModerationHandlers(socket: AuthenticatedSocket): void {
  ...
  // Ban user
  socket.on('admin:banUser', async (data: {
    userId: number;
    banType: BanType;
    reason: string;
    duration?: number;
  ...
  // Mute user
  socket.on('admin:muteUser', async (data: {
    userId: number;
    duration: number;
    reason: string;
  }, callback) => {
  ...
  // Kick user
  socket.on('admin:kickUser', async (data: {
    userId: number;
    reason: string;
  }, callback) => {
    try {
  ...
  // Delete post
  socket.on('admin:deletePost', async (data: {
    postId: number;
    reason: string;
  }, callback) => {
    try {
  ...
  socket.on('admin:getModerationHistory', async (data: {
    targetUserId?: number;
    moderatorId?: number;
  }, callback) => {
    try {
