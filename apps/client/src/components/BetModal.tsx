...
export default function BetModal({ prediction, isOpen, onClose, mode, onBetPlaced }: BetModalProps) {
  ...
  const getRiskColors = () => {
    switch (betCalculations.excitementLevel) {
      case 'yolo': return 'bg-gradient-to-br from-red-600 to-red-700 text-white';
      case 'high': return 'bg-gradient-to-br from-orange-600 to-orange-700 text-white';
      case 'aggressive': return 'bg-gradient-to-br from-yellow-600 to-yellow-700 text-white';
      case 'moderate': return 'bg-gradient-to-br from-blue-600 to-blue-700 text-white';
      default: return 'bg-gradient-to-br from-green-600 to-green-700 text-white';
    }
  };

  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className={`bg-surface border border-muted rounded-2xl shadow-2xl ${modalSize} ${spacing} relative overflow-hidden`}>
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1 pr-4">
            <h3 className={`font-bold ${mode === 'compact' ? 'text-lg' : 'text-xl'} text-content line-clamp-2`}>
              {prediction.title}
            </h3>
            <div className="text-sm text-tertiary mt-1">
              Balance: <span className="font-semibold text-primary">{balance.toLocaleString()} 🪙</span>
            </div>
          </div>
          ...
          {amount > 0 && (
            <div className={`px-3 py-1 rounded-full text-xs font-bold ${
              betCalculations.excitementLevel === 'yolo' ? 'bg-red-600/20 text-red-600 animate-pulse' :
              betCalculations.excitementLevel === 'high' ? 'bg-orange-600/20 text-orange-600' :
              betCalculations.excitementLevel === 'aggressive' ? 'bg-yellow-600/20 text-yellow-600' :
              betCalculations.excitementLevel === 'moderate' ? 'bg-blue-600/20 text-blue-600' :
          ...
          <div>
            ...
            <input
              ...
              className={`w-full border border-muted p-3 rounded-lg bg-background text-content focus:outline-none focus:ring-2 focus:ring-primary transition-all ${
                betCalculations.excitementLevel === 'yolo' ? 'ring-2 ring-red-500 border-red-500' : ''
              }`}
              disabled={placing}
        ...
        {amount > 0 && (
          <div className={`rounded-lg p-4 space-y-3 transition-all duration-300 ${
            betCalculations.excitementLevel === 'yolo' ? 'bg-red-600/5 border border-red-600/20' :
            betCalculations.excitementLevel === 'high' ? 'bg-orange-600/5 border border-orange-600/20' :
            'bg-muted'
          }`}>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="flex justify-between">
                <span>Profit:</span>
                <span className="font-bold text-green-600">+{betCalculations.profitPercent.toFixed(0)}%</span>
              </div>
            </div>
            ...
            <div className="flex justify-between text-base font-bold border-t border-muted pt-2">
              ...
              <span className={`transition-all duration-300 ${
                betCalculations.excitementLevel === 'yolo' ? 'text-red-600 animate-pulse text-lg' :
                betCalculations.excitementLevel === 'high' ? 'text-orange-600' :
                'text-green-600'
              }`}>
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className={betCalculations.balanceAfter < balance * 0.2 ? 'text-orange-600 font-semibold' : 'text-tertiary'}>
            ...
            {betCalculations.isAllIn && (
              <div className="text-center py-3 bg-gradient-to-r from-red-600/10 to-orange-600/10 border border-red-600/20 rounded-lg">
                <div className="text-red-600 font-bold text-lg animate-pulse">
                  🚀 ALL-IN BONUS: {((betCalculations.allInMultiplier - 1) * 100).toFixed(0)}% EXTRA PAYOUT! 🚀
                </div>
        ...
        {/* Error Display */}
        {err && (
          <div className="text-xs text-red-600 bg-red-600/10 p-3 rounded-lg">
            {err}
          </div>
        )}
          ...
          >
            {placing ? (
              ...
              <span className="flex items-center justify-center space-x-2">
                ...
                <span>
                  {betCalculations.isYolo ? 'SEND IT! 🚀' : 
                   betCalculations.excitementLevel === 'high' ? 'HIGH RISK BET!' :
                   betCalculations.excitementLevel === 'aggressive' ? 'AGGRESSIVE BET!' :
                   'Place Bet'}
                </span>
              </span>
            )}
